...
export function createAppsRouter(context: AppContext) {
  const router = express.Router();
  const { knex, logger } = context;
  const middleware = createMiddleware(knex, logger);
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      ...
      await knex("apps").where({ id, user_id: req.session?.user?.id }).update({
        is_active,
        name,
        url,
        description,
  ...
  router.post(
    ...
    middleware.csrfMiddleware,
    async (req: Request, res: Response) => {
      await knex("apps").where({ id: req.params.id, user_id: req.session?.user?.id }).del();
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      ...
      const app = await knex("apps").where({ id, user_id: req.session?.user?.id }).first();
      ...
      await knex("apps").where({ id, user_id: req.session?.user?.id }).update({
        ...
        api_key_created_at: knex.fn.now(),
      });
      ...
      const channels = await Promise.all(
        channelsRaw.map(async (channel) => {
          ...
          if (channel.channel_type === "email") {
            config = await knex("email_configs").where("app_channel_id", channel.id).first();
          } else if (channel.channel_type === "sms") {
            config = await knex("sms_configs").where("app_channel_id", channel.id).first();
          } else if (channel.channel_type === "discord") {
            config = await knex("discord_configs").where("app_channel_id", channel.id).first();
          }
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      ...
      const app = await knex.select("*").from("apps").where({ id: appId, user_id: userId }).first();
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      const { id } = req.params;
      const { name, is_active, account_sid, auth_token, from_phone_number, phone_number } =
        req.body;
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      const channel = await knex("app_channels")
