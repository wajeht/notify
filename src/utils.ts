...
export async function runMigrations() {
	try {
		...
		const pendingMigrations = await db.migrate.list(config);
		...
		console.log(`Running ${pendingMigrations[1].length} pending migrations...`);
		...
		if (migrations.length === 0) {
			...
			const migrationList = migrations
				.map((migration: any) => migration.split('_')[1].split('.')[0])
				.join(', ');
			console.log(`Completed migrations: ${migrationList}`);
			console.log(`Batch ${batchNo} run: ${migrations.length} migrations`);
		}
	} catch (error) {
